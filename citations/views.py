...
def render_rights(data: dict) -> str:
    ...
    if data.get("rights") in settings.RIGHTS_MAP:
        return (
            f'<a href={settings.RIGHTS_MAP[data["rights"]][0]}>'
...
def render_reference_html(ref: dict) -> dict:
    ...
    ref["citeas"] = (
        ref["citeas"]
        .replace(ref["title"], f"<b>{ref["title"]} </b>")
        .replace(ref["id"], f"<a href={ref['id']}>{ref['id']}.</a>")
    )
...
def filter_versions(queryset):
    ...
    for t in sorted(titles):
        instances.append(queryset.filter(title=t).order_by("version").last())
    return instances
...
class PaginatedListView(GenericRenderedView):
    ...
    def get_context_data(
        self, page_obj, page_number: int = 1, search_term=None, count=None, **kwargs
    ):
        """
    ...
    def perform_search(self) -> tuple:
        ...
        if (
            ...
            and self.request.GET.get("search") is not None
        ):
            term = self.request.GET.get("search")
            searches = deep_search(
...
class CitationsView(PaginatedListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["all_versions"] = bool(
            self.request.GET.get("legacy_versions", "") != ""
        )
        return context
...
class CitationView(GenericRenderedView):
    ...
    def get_context_data(self, title, **kwargs):
        ...
        if not version and self.request.GET.get("version"):
            ...
            if not version:
                citation = (
                    Citations.objects.filter(title=title).order_by("version").last()
                )
            else:
...
class FundingStreamView(GenericRenderedView):
    ...
    def get_context_data(self, pk, **kwargs):
        ...
        context["stream"]["affiliation_id"] = self.model.objects.get(
            pk=pk
        ).affiliation.id

        context["citations"] = (
            Citations.objects.filter(funders__id=pk)
            .values_list("title", flat=True)
            .distinct()
        )
...
class SpecificCitationAPIView(SpecificAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if self.model.objects.filter(title=pk):
            self.kwargs["pk"] = (
                self.model.objects.filter(title=pk).order_by("-version").last().pk
            )
...
class CitationFormMixin(PermissionRequiredMixin, GenericRenderedView, FormView):
    ...
    def replicate_data(self, instance=None, data=None):
        ...
        serializer.is_valid(raise_exception=True)
        obj = serializer.save(publish=publish, user_id=self.request.user.username)
...
class ReviewerRequestView(LoginRequiredMixin, GenericRenderedView, FormView):
    ...
    def get_user_permissions_text(self):
        ...
        for i in self.request.user.user_permissions.values_list("codename", flat=True):
            if "edit" in i:
                perms.append(
                    f'{i} (Edit citations belonging to Institution: {i.replace("edit_","")})'
                )
        return perms
    ...
    def form_valid(self, form):
        ...
        change_permissions = not self.request.user.user_permissions.filter(
            pk=general.pk
        )
        ...
        if change_permissions:
            request_text = (
                f":bell: Github user: {self.request.user.username} ({self.request.user.first_name} {self.request.user.last_name}) "
            ...
            messages.success(
                self.request,
                f'Your request for permission to edit Citation records for {", ".join(institutions)} has been submitted.',
            )
        else:
